...
- **Disposition**: COMPLETED - join_condtion is now built unconditionally from conflict_fields before the hash_fields branch, so the trailing UPDATE keeps its WHERE, src/pyutilz/database/db/upsert.py:206-213
...
- **Disposition**: COMPLETED - both read sites use the calling thread's own cursor via get_cursor(get_cursor_type(None, None)), src/pyutilz/database/db/__init__.py:141-149, 442-446
...
- **Disposition**: COMPLETED - _regjobs_update binds psycopg2.extras.Json(result) for dict/list payloads, src/pyutilz/database/db/__init__.py:1219-1220
...
- **Disposition**: COMPLETED - the DSN is compared before any pool is returned, on both the fast path and the double-checked path, and in get_connection_from_pool, src/pyutilz/database/psycopg2_pool.py:74-88, 197-198
...
- **Disposition**: COMPLETED - the fixed port is captured once into fixed_port and a fresh port is drawn on every iteration, src/pyutilz/web/web.py:530-540
...
- **Disposition**: COMPLETED - the loop pairs each dict with its own threshold, so failed_dict is compared against min_failed_idle_interval_minutes, src/pyutilz/web/web.py:547-553
- **Category**: proxy-rotation
...
- **Disposition**: COMPLETED - download_to_file re-issues the GET on every attempt, closes each response, and removes the partial file after the last failure, src/pyutilz/web/web.py:978-1016
...
- **Disposition**: COMPLETED - all 13 OpenRouter per-call fields are PerCallAttr descriptors and are listed in the class's own _PERCALL_METADATA_ATTRS, so generate_batch's yielded dict carries them, src/pyutilz/llm/openrouter_provider/_provider.py:77-115
- **Category**: concurrency
...
- **Disposition**: COMPLETED - _last_json_schema_applied is a PerCallAttr and is included in OpenAICompatibleProvider._PERCALL_METADATA_ATTRS, src/pyutilz/llm/openai_compat.py:186-191
- **Category**: concurrency
...
- **Disposition**: COMPLETED - the stream accumulates the latest usage block into a local and records it once after the stream closes, guarded by a per-call usage_recorded flag, src/pyutilz/llm/openai_compat.py:549-597
...
- **Disposition**: COMPLETED - eviction closes only a provider with no remaining external strong reference (sys.getrefcount baseline); otherwise the LRU entry is dropped and the WeakSet plus atexit handler tear it down, src/pyutilz/llm/factory.py:198-210
...
- **Disposition**: COMPLETED - range(long_len - short_len + 1) in the pure-Python loop and all four numba matrix-fill kernels, src/pyutilz/text/similarity.py:267, 500, 681, 982, 1074
- **Category**: off-by-one
...
- **Disposition**: COMPLETED - entropy is normalized by sum(stats.values()), the total observation count, src/pyutilz/text/strings/textentropy.py:108-111
...
- **Disposition**: COMPLETED - DSN_PASSWORD_RE and DSN_SCHEME_RE accept an empty username, and both Bearer and Basic are handled by TELEGRAM_TOKEN_RE and excluded from SECRET_KEY_VALUE_RE, src/pyutilz/text/secrets_scrub.py:38, 50, 55-66, 82
...
- **Disposition**: COMPLETED - a falsy level is normalized to 'info' at entry, so the message is always logged and persisted, src/pyutilz/database/db/__init__.py:583-587
...
- **Disposition**: COMPLETED - build_upsert_query raises ValueError when history_table_name is set without history_fields, src/pyutilz/database/db/upsert.py:79-84
- **Category**: sql-building
...
- **Disposition**: COMPLETED - mode='insert' now requires a non-empty set_fields, mirroring the select/update guard, src/pyutilz/database/db/__init__.py:487-491
...
- **Disposition**: COMPLETED - release_connection closes a connection when the pool is gone instead of abandoning its backend, src/pyutilz/database/psycopg2_pool.py:257-263
- **Category**: connection-lifecycle
...
- **Disposition**: COMPLETED - basic_db_execute logs a warning when auto_commit=False, naming the autocommit isolation level that makes the flag inert, src/pyutilz/database/db/__init__.py:304-307
...
- **Disposition**: COMPLETED - the 429 branch honours a Retry-After header and otherwise applies the same jittered delay as the other two branches, via the new _parse_retry_after helper, src/pyutilz/web/web.py:812-820, 940-960
...
- **Disposition**: COMPLETED - LoginAndGetCookies builds its headers from a copy of basic_headers and drops any inherited authorization, src/pyutilz/web/browser.py:529-536
...
- **Disposition**: COMPLETED - _cache_path takes a kind that participates in the key; get_text stores under the text key while the legacy digest-only key stays valid for json, src/pyutilz/web/cached_client.py:101-112, 196
- **Category**: caching
...
- **Disposition**: COMPLETED - the JSON body is decoded with errors='replace' (matching get_text), so a mis-encoded response no longer aborts the batch, src/pyutilz/web/cached_client.py:178-181
- **Category**: encoding
...
- **Disposition**: COMPLETED - report_success trims the window too, src/pyutilz/web/proxy/base.py:206-213
- **Category**: resource-lifecycle
...
- **Disposition**: COMPLETED - get_traffic paginates until a short page (bounded by _MAX_TRAFFIC_PAGES) and warns if the cap is hit; row extraction factored into _extract_traffic_rows, src/pyutilz/web/proxy/decodo.py:365-401, 421-434
- **Category**: pagination
...
- **Disposition**: COMPLETED - generate() clears _last_result_message at the top of every call, src/pyutilz/llm/claude_code_provider.py:359-364
- **Category**: token-accounting
...
- **Disposition**: COMPLETED - every parse path goes through the new _require_json_object, which raises JSONParsingError for a non-object, src/pyutilz/llm/base.py:216-228, 385, 405, 429
...
- **Disposition**: COMPLETED - GeminiProvider overrides generate_json to pass json_mode=True through _generate_json_via, src/pyutilz/llm/gemini_provider.py:172-187
...
- **Disposition**: COMPLETED - the MAX_TOKENS check runs before the empty-text branches, so a truncated response raises LLMTruncationError rather than LLMSafetyBlockError, src/pyutilz/llm/gemini_provider.py:296-304
...
- **Disposition**: COMPLETED - Anthropic's get_session_cost returns total_cost_usd, and GeminiProvider gained get_session_cost (billing cached prompt tokens at the previously-dead _CACHE_HIT_COST rate) plus the cumulative counters it needs, src/pyutilz/llm/anthropic_provider.py:361-386, src/pyutilz/llm/gemini_provider.py:189-219
...
- **Disposition**: COMPLETED - LLMTruncationError carries partial_text, the content access is guarded against an empty list and a non-text block 0, and a missing text block raises LLMProviderError, src/pyutilz/llm/anthropic_provider.py:236-261
...
- **Disposition**: COMPLETED - generate() warns once per parameter (via the new _warn_unsupported_param_once) when max_tokens or a non-default temperature cannot be forwarded to the SDK, src/pyutilz/llm/claude_code_provider.py:335-347, 365-372
...
- **Disposition**: COMPLETED - generate_json delegates to self.extract_json, so refusals and parse failures raise LLMRefusalError/JSONParsingError instead of a bare ValueError, src/pyutilz/llm/claude_code_provider.py:723-729
- **Category**: error-classification
...
- **Disposition**: COMPLETED - _fetch_models_catalogue never issues its blocking httpx.get from an event-loop thread; it serves the cached (possibly stale) catalogue instead and lets the off-loop _ensure_catalogue_warm_async refresh it, src/pyutilz/llm/openrouter_provider/_catalogue.py:63-82
...
- **Disposition**: COMPLETED - the SDK client is constructed with max_retries=0 and an explicit timeout (_request_timeout_seconds), leaving _retry.py as the single retry authority, src/pyutilz/llm/anthropic_provider.py:24-28, 89-93
...
- **Disposition**: COMPLETED - the three missing _CACHE_HIT_COST entries and two missing _MAX_TOKENS/_CONTEXT_WINDOW entries were added, and OpenAIProvider resolves both properties via longest_prefix_lookup; the base properties keep exact lookup because xAI's default is deliberately larger than its listed entries, src/pyutilz/llm/openai_provider.py:64-66, 84-85, 106-107, 194-207
...
- **Disposition**: COMPLETED - NUL is stripped from the OBJECT (str values and keys) in the renamed _normalize_for_pg_json, and the escape-editing string replace on the serialized text is gone, src/pyutilz/text/strings/jsonutils.py:235-261, 285
- **Category**: encoding
...
- **Disposition**: COMPLETED - remove_videos returns its input unconditionally and sentencize_text returns early on empty/whitespace-only text, src/pyutilz/text/strings/webtext.py:45-47, 381-385
- **Category**: edge-case
...
- **Disposition**: COMPLETED - unescape_html uses html.unescape (full HTML5 named plus numeric set), src/pyutilz/text/strings/webtext.py:63-68
- **Category**: encoding
...
- **Disposition**: COMPLETED - FIRSTLETTER_CAPITAL/ALLLETTERS_CAPITAL are computed once per word before the (i, j) loops, src/pyutilz/text/tokenizers.py:155-161
- **Category**: edge-case
...
- **Disposition**: COMPLETED - the min-length guard runs before the prefix branch, and the 0.9 floor requires at least a 2-character common prefix, in the pure-Python path and all four numba kernels, src/pyutilz/text/similarity.py:250-258, 471-483, 652-664, 955-967, 1047-1059
...
- **Disposition**: COMPLETED - path is validated against a JSON-navigation regex (_JSON_PATH_RE) before being spliced into the query, src/pyutilz/database/db/__init__.py:598-599, 1093-1098
- **Category**: sql-building
...
- **Disposition**: COMPLETED - a None returning is normalized to the empty string at entry, and select mode defaults it back to '*', src/pyutilz/database/db/__init__.py:492-496
- **Category**: input-validation
...
- **Disposition**: COMPLETED - the except branch sets conn = None after closing, so get_cursor's assert reports the intended diagnostic, src/pyutilz/database/db/__init__.py:253-257
- **Category**: connection-lifecycle
...
- **Disposition**: COMPLETED - the doubled list is gone and the assert became a real ValueError, src/pyutilz/database/db/upsert.py:239-244
- **Category**: sql-building
...
- **Disposition**: COMPLETED - _conn_key also catches psycopg2.Error and treats a falsy backend_pid as unavailable, falling back to id(conn), src/pyutilz/database/psycopg2_pool.py:60-70
- **Category**: connection-lifecycle
...
- **Disposition**: COMPLETED - the non-MySQL branch logs a warning naming the flavor, src/pyutilz/database/db/__init__.py:967-969
- **Category**: silent-failure
...
- **Disposition**: COMPLETED - get_url applies the same 10s floor the urlopen sites use when timeout is None, src/pyutilz/web/web.py:718-722
...
- **Disposition**: COMPLETED - the PROXY_PASS check uses truthiness, matching its sibling guards, src/pyutilz/web/browser.py:205-207
- **Category**: input-validation
...
- **Disposition**: COMPLETED - PROXY_PORT_RANGE gets the same try/except with the variable name and value in the message, src/pyutilz/web/proxy/decodo.py:250-255
...
- **Disposition**: COMPLETED - a provider returning no usable payload is logged at the isinstance check, where the failure actually lands, src/pyutilz/web/web.py:245-248
- **Category**: dead-code
...
- **Disposition**: COMPLETED - close_browser() is called before restarting Selenium on the browser_get failure path, src/pyutilz/web/browser.py:409-413
- **Category**: resource-lifecycle
...
- **Disposition**: COMPLETED - the base _reset_per_call_state resets all five PerCallAttr fields instead of being a no-op, and OpenRouter's override calls super() first, src/pyutilz/llm/openai_compat.py:269-282 and src/pyutilz/llm/openrouter_provider/_provider.py:196
...
- **Disposition**: COMPLETED - xAI resolves pricing by longest prefix and warns once per unrecognized model via _warn_unknown_model_once; grok-3 and grok-3-mini were added to _MAX_TOKENS, src/pyutilz/llm/xai_provider.py:34-35, 92-135
- **Category**: cost-accounting
...
- **Disposition**: COMPLETED - the fraction-vs-percentage shape is decided once per payload by _uptime_payload_is_percentage instead of per value, so a genuine 1.0 percent is no longer read as 100 percent, src/pyutilz/llm/openrouter_provider/_health.py:130-152, 176-181, 210-212
...
- **Disposition**: COMPLETED - the reset-date construction is wrapped in try/except ValueError and falls through to the caller's default wait, src/pyutilz/llm/claude_code_provider.py:210-224
- **Category**: error-handling
...
- **Disposition**: COMPLETED - resolved by F08: every field last_call_summary reads is now a PerCallAttr, so the whole dict is one context's own call rather than a mix of real values and defaults; documented in the method's docstring, src/pyutilz/llm/openrouter_provider/_provider.py:754-759
- **Category**: concurrency
...
- **Disposition**: COMPLETED - the three OrderedDict mutations are guarded by _encoding_cache_lock, with the potentially network-hitting encoding resolve kept outside the lock, src/pyutilz/llm/token_counter.py:42-47, 50-76
- **Category**: concurrency
...
- **Disposition**: COMPLETED - dict.fromkeys gives order-stable dedup and empty tokens are filtered out, src/pyutilz/text/similarity.py:386-389
...
- **Disposition**: COMPLETED - idx2 is None distinguishes an explicit 0 from unset, and the empty-start branch honours idx1, src/pyutilz/text/strings/basics.py:20-28
- **Category**: edge-case
...
- **Disposition**: COMPLETED - the space collapse is scoped to the neighbourhood of the substituted dash, src/pyutilz/text/humanizer.py:140-145
...
- **Disposition**: COMPLETED - the password run is bounded to non-whitespace so a greedy match cannot span two DSNs, while still anchoring to the last @ within one token, src/pyutilz/text/secrets_scrub.py:38
- **Category**: over-broad-regex
...
- **Disposition**: COMPLETED - both flags are computed once from the word rather than at every start offset, src/pyutilz/text/tokenizers.py:155-161
- **Category**: algorithm-correctness
