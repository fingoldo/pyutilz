...
def _param_backed_attributes(init: ast.FunctionDef | ast.AsyncFunctionDef) -> dict[str, str]:
    ...
    # `_arg_names` rather than args+kwonlyargs: PEP 570 positional-only parameters

    # (`def __init__(self, rate, /)`) are valid on this project's 3.8 floor and were invisible here.

    params = set(_arg_names(init))
...
def _assigns_by_method(methods: list, init: ast.AST, bound: dict) -> "dict[str, list[tuple[str, int]]]":
    """{method name -> [(constructor-bound attribute, line)]} for every non-``__init__`` assignment.


    Which method assigns which constructor-bound attribute, and which methods read configuration,

    are frequently NOT the same method: `_TokenBucket`, the case this rule was written for, reads

    `cfg()` in `_refresh_rate` and assigns `self._rate` inside `update_rate(rate)` one call further

    down. Requiring both in one statement missed this rule's own worked example.
    ...
    for method in methods:
        ...
        for stmt in ast.walk(method):
            ...
            for attr in _self_attr_targets(stmt) & set(bound):
                ...
                out.setdefault(method.name, []).append((attr, stmt.lineno))

    return out


def _overwrite_candidates(methods: list, init: ast.AST, bound: dict) -> "list[tuple[str, str, str, int]]":
    """(reader method, via method, attribute, line) for every config-sourced overwrite.


    The config read has to REACH the value being stored -- either it is in the assigned expression

    itself, or it is in an argument handed to the sibling method that stores it (`_refresh_rate`

    reading `cfg()` and calling `update_rate(new)`, the worked example). Accepting any mention of

    `config`/`settings`/`environ` anywhere in the method reported a method that merely LOGS

    `self.config` and separately assigns an unrelated parameter.

    """

    assigns = _assigns_by_method(methods, init, bound)
    ...
    for method in methods:
        ...
        for stmt in ast.walk(method):
            ...
            if stmt.value is None or not _reads_config(stmt.value):

                continue

            for attr in sorted(_self_attr_targets(stmt) & set(bound)):

                candidates.append((method.name, method.name, attr, stmt.lineno))
        ...
        for call in ast.walk(method):
            ...
            if not any(_reads_config(arg) for arg in [*call.args, *(kw.value for kw in call.keywords)]):

                continue

            for attr, line in assigns.get(call.func.attr, []):

                candidates.append((method.name, call.func.attr, attr, line))
...
def scan_constructor_param_overwritten(
    ...
    for py in _iter_py_files(root, exclude_dirs):
        ...
        for cls in ast.walk(tree):
            ...
            for method_name, via, attr, stmt_line in sorted(_overwrite_candidates(methods, init, bound)):

                if (attr, stmt_line) in reported:

                    continue

                reported.add((attr, stmt_line))

                through = "" if via == method_name else f" (via `{via}`)"
                ...
                findings.append(
                    Finding(
                        ...
                        detail=(
                            ...
                            f"`self.{attr}`, and `{method_name}` reassigns it from "
