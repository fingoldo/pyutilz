...
Lives in the ``llm`` domain (it was previously ``pyutilz.core.openai``, the one and only
import edge from the base ``core`` layer up into ``llm``, which stopped ``core`` being a
leaf). ``pyutilz.core.openai`` remains importable as a deprecated shim.
