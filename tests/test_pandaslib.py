...
class TestConcatAndFlushDfList:
    ...
    def test_csv_with_none_cols_parameter(self, tmp_path):
        ...
        result = concat_and_flush_df_list(df_list, file_name=str(csv_path), to_csv=True, csv_cols=None)
    ...
    def test_pickle_output(self, tmp_path):
        ...
        result = concat_and_flush_df_list(df_list, file_name=str(pickle_path), to_csv=False)
...
class TestNullifyStandardValues:
    ...
    def test_with_persons_field(self):
        ...
        df = pd.DataFrame({"field": ["A"] * 100 + ["B"] * 100, "person": [1] * 50 + [2] * 50 + list(range(3, 103))})

        nullify_standard_values(df, "field", min_records=50, persons_field="person", min_persons=10)
