...
class TestShowcaseDfColumns:
    ...
    def test_polars_branch_collect_all_called_once(self):
        ...
        # observed failing on CI after test_meta's importlib.reload(pyutilz) runs earlier in the
        # same session, even though `import pyutilz.data.pandaslib.frames` always succeeds
        # directly). Previously fixed in e0cbbd4, silently reverted by a later stale-local-copy
        # merge (7bcea77) -- reapplied.
