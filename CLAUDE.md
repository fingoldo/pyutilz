...
**Black repo-wide reformat — resolved (2026-07-05):** user decided to exclude exactly two Black behaviors from any run, project-wide (mirrored in mlframe): (1) arg/collection-list explosion (multi-item packed line -> one-item-per-line, including `from x import (...)` blocks) and (2) blank-line insertion. Neither is configurable via a stock Black flag. Originally `scripts/black_filtered_apply.py`; extracted 2026-07-09 into the shared `py_ci_shared` package (https://github.com/fingoldo/py-ci-shared, installed via `pip install -r requirements-dev.txt` -- NOT the `[dev]` extra: a `git+https` direct reference in project metadata makes the distribution unpublishable on PyPI) so pyutilz and mlframe consume the identical script instead of drifting copies. Applies everything else Black wants while mechanically rejecting those two, validated via AST-equivalence + compile checks. Use `python -m py_ci_shared.black_filtered_apply --config pyproject.toml --write <files>` (or `--check .` for CI/dry-run) — never raw `black`/`black --fix` in place.
...
## Never trim a `# type: ignore` code just because it looks unused locally (2026-09-03)

Whether mypy reports an error at all can depend on which optional packages the checking environment has. `src/pyutilz/llm/openai_compat.py` and `src/pyutilz/web/proxy/ip_check.py` alias an optional import (`import orjson as _json_backend`, with a stdlib fallback): where orjson does not resolve, mypy reports `import-not-found`; where it does, it reports `no-redef` against the annotation above instead. Both codes must stay on those lines. The same applies to the numba `prange` and BeautifulSoup `find_all` ignores nearby.

**Why:** this has now been reverted twice - once by a cleanup that removed codes measured as unused on a dev box, once by a merge resolution - and both times CI went red on a gate that had been green locally. A local mypy run passing with the codes REMOVED proves nothing, because the dev box is exactly the environment where they do not fire.

**How to apply:** treat a `# type: ignore[a,b]` whose comment names the environment each code is for as load-bearing; do not narrow it. If a tool or audit reports one of those codes as unused, that is the tool speaking for one environment only. `warn_unused_ignores` is deliberately off for this reason (2026-09-02 audit, 06/F10).
